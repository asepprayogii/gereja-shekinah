...
class JemaatController extends Controller
{
    public function index(Request $request)
    {
        ...
        $jemaat = $query->orderBy('nama_lengkap')
                        ->paginate(20)
                        ->withQueryString();
    ...
    public function store(Request $request)
    {
        ...
        // Kosongkan string kosong jadi null untuk field tanggal
        foreach (['tanggal_lahir', 'tanggal_baptis', 'tanggal_sidi'] as $field) {
            if (empty($data[$field])) $data[$field] = null;
        }

        if ($request->hasFile('foto')) {
            $data['foto'] = $request->file('foto')->store('jemaat-foto', 'public');
        }

        Jemaat::create($data);

        return redirect()->route('admin.jemaat')
    ...
    public function update(Request $request, $id)
    {
        ...
        if ($request->hasFile('foto')) {
            if ($jemaat->foto) Storage::disk('public')->delete($jemaat->foto);
            $data['foto'] = $request->file('foto')->store('jemaat-foto', 'public');
        }
    ...
    public function export()
    {
        ...
        $callback = function () use ($jemaat) {
            ...
            fputcsv($f, [
                ...
                'Status Pernikahan', 'Pekerjaan', 'No HP',
                'Alamat', 'Nama Keluarga',
                'Tanggal Baptis', 'Tanggal Sidi',
            ...
            foreach ($jemaat as $j) {
                fputcsv($f, [
                    $j->nama_lengkap,
                    $j->jenis_kelamin,
                    $j->tanggal_lahir ? $j->tanggal_lahir->format('Y-m-d') : '',
                    $j->status_pernikahan,
                    $j->pekerjaan,
                    $j->no_hp,
                    $j->alamat,
                    $j->nama_keluarga,
                    $j->tanggal_baptis ? $j->tanggal_baptis->format('Y-m-d') : '',
                    $j->tanggal_sidi   ? $j->tanggal_sidi->format('Y-m-d')   : '',
                    $j->catatan,
    ...
    public function template()
    {
        ...
        $callback = function () {
            ...
            fputcsv($f, [
                ...
                'alamat',
                'nama_keluarga',
                'tanggal_baptis(YYYY-MM-DD)',
                'tanggal_sidi(YYYY-MM-DD)',
                'catatan',
            ]);
            fputcsv($f, [
                'Budi Santoso', 'Laki-laki', '1980-05-15',
                'Menikah', 'Wiraswasta', '081234567890',
                'Jl. Contoh No.1', 'Keluarga Santoso',
                '2000-01-01', '', '',
            ]);
            fputcsv($f, [
                'Siti Rahayu', 'Perempuan', '1983-08-20',
                'Menikah', 'Ibu Rumah Tangga', '081298765432',
                'Jl. Contoh No.1', 'Keluarga Santoso',
                '2001-06-10', '', '',
            ]);
            fclose($f);
    ...
    public function import(Request $request)
    {
        ...
        // Urutan kolom:
        // 0:nama_lengkap, 1:jenis_kelamin, 2:tanggal_lahir,
        // 3:status_pernikahan, 4:pekerjaan, 5:no_hp,
        // 6:alamat, 7:nama_keluarga, 8:tanggal_baptis, 9:tanggal_sidi, 10:catatan
        ...
        while (($row = fgetcsv($handle)) !== false) {
            ...
            try {
                ...
                Jemaat::create([
                    'nama_lengkap'      => $nama,
                    'jenis_kelamin'     => $jk,
                    'tanggal_lahir'     => $parseDate($row[2] ?? ''),
                    'status_pernikahan' => $sp,
                    'pekerjaan'         => trim($row[4] ?? ''),
                    'no_hp'             => trim($row[5] ?? ''),
                    'alamat'            => trim($row[6] ?? ''),
                    'nama_keluarga'     => trim($row[7] ?? ''),
                    'tanggal_baptis'    => $parseDate($row[8] ?? ''),
                    'tanggal_sidi'      => $parseDate($row[9] ?? ''),
                    'catatan'           => trim($row[10] ?? ''),
                    'status_aktif'      => 1,
                ]);
                $berhasil++;
